...
import CareersComponent from '../components/CareersComponent.vue';
import { careers } from '../data/careers';

describe('CareersComponent.vue', () => {
  beforeEach(() => {
    window.open.mockClear();
  });

  it('renders #dialog-careers with nes-dialog class', () => {
    const wrapper = mount(CareersComponent);
    const dialog = wrapper.find('#dialog-careers');
  ...
  it('close button closes careers dialog', async () => {
    const container = document.createElement('div');
    document.body.appendChild(container);
    const wrapper = mount(CareersComponent, { attachTo: container });
    ...
    const closeButton = wrapper
      .findAll('.dialog-menu button')
      .filter((b) => b.text() === 'Okay')[0];
